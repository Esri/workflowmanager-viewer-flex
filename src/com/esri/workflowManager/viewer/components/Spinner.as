package com.esri.workflowManager.viewer.components
{
	...
	public class Spinner extends UIComponent
	{
		...
		private static function classConstruct():Boolean {
			if (!StyleManager.getStyleManager(null).getStyleDeclaration("Spinner")) {
				// If there is no CSS definition for Spinner, 
				// then create one and set the default value.
				var newStyleDeclaration:CSSStyleDeclaration = new CSSStyleDeclaration();
				newStyleDeclaration.setStyle(STYLE_TICK_COLOR, 0x000000);
				StyleManager.getStyleManager(null).setStyleDeclaration("Spinner", newStyleDeclaration, true);
			}
			return true;
		}
